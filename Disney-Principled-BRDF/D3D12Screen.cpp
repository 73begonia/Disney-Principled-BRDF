...
D3D12Screen::D3D12Screen(HWND hwnd, UINT width, UINT height) : mTargetWindow(hwnd), mScreenW(width), mScreenH(height)
{
...
void D3D12Screen::initializeResource()
{
	...
	mSrvUavDescriptorSize = mDevice_v0->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	srvHandle.ptr += (UINT)DescriptorID::tracerOutTextureSRV * mSrvUavDescriptorSize;
	mDevice_v0->CreateShaderResourceView(mTracerOutTexture.Get(), nullptr, srvHandle);
}
...
void D3D12Screen::fillCommandLists()
{
	...
	tracerOutTextureHandle.ptr += (UINT)DescriptorID::tracerOutTextureSRV * mSrvUavDescriptorSize;
	mCmdList_v0->SetGraphicsRootDescriptorTable(0, tracerOutTextureHandle);
...
void D3D12Screen::display(const TracedResult& trResult)
{
	...
	UINT8* pBufs = nullptr;
	mTextureUploader->Map(0, nullptr, reinterpret_cast<void**>(&pBufs));
